...
pointing away from $v$.  An oriented graph is \emph{acyclic} if there is no sequence of arrows that ends where it starts.
...
we may pick such an $L$ and $G$ with $\card{G}$ as small as possible.  Since $\card{G}$ is finite and $G$ is acyclic, there must be $w \in V(G)$ with $d^+(w) = 0$.
...
$\card{L'(v)} > d_{G-w}^+(v)$ for all $v \in V(G-w)$.  Since $\card{G-w} < \card{G}$, applying the theorem shows that $G-w$ is $L'$-colorable, but then we have an $L$-coloring
of $G$, a contradiction.
\end{proof}
...
But there are ways to replace ``acyclic'' with weaker hypotheses and still get a true theorem.


It will often be convenient to work with an equivalent dual version of list assignments.  
