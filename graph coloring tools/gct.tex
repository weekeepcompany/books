...
Let $s,t \in V$ be two distinguished vertices.  The \emph{capacity} of a network $f$, written $\card{f}$, is the minimum of $\card{X, V}_f$ over all $X \subseteq V\setminus\set{t}$ with $s \in X$.
...
\end{lemma}
\begin{proof}
Pick $X \subseteq V\setminus\set{t}$ with $s \in X$ such that $\card{X, V}_g = \card{g}$.  Then, by Lemma \ref{FlowValues}, $\size{f} = \card{X, V}_f - \card{V, X}_f \le \card{X, V}_g = \card{g}$.
\end{proof}
